...
const AdherenceReportOutputSchema = z.object({
  ...
  summaryNotes: z.string().describe('AI-generated summary and counseling notes for the patient or pharmacist based on the adherence rates.'),
...
2. For each medication, calculate the 'adherenceRate' as a percentage: (dosesTaken / dosesPrescribed) * 100. Round to the nearest whole number and format as a string (e.g., "85%").
3. Calculate the 'overallAdherence' rate for all medications combined. This should be the percentage of total doses taken out of total doses prescribed. Round and format as a string.
4. Based on the adherence rates, generate a concise 'summaryNotes' section. If overall adherence is less than 80%, provide gentle counseling points and suggest talking to a pharmacist. If it's good, provide encouragement.
