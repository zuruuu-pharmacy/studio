...
const formSchema = z.object({
  ...
  patientMedicalHistory: z.string().optional(),
});
...
export function AllergyClient() {
  const [state, formAction, isPending] = useActionState<AllergyCheckerOutput | { error: string } | null, FormData>(
    async (previousState, formData) => {
      ...
      try {
        const result = await allergyChecker(parsed.data);
        return result;
      } catch (e) {
        console.error(e);
    ...
    <div className="grid md:grid-cols-3 gap-6">
          ...
          <CardContent>
            <Form {...form}>
              <form action={formAction} className="space-y-4">
                ...
                <FormField name="patientMedicalHistory" control={form.control} render={({ field }) => (
                  <FormItem><FormLabel>Patient Medical History (Optional)</FormLabel><FormControl><Textarea placeholder="e.g., Asthma, Renal Impairment" {...field} /></FormControl><FormMessage /></FormItem>
                )} />
      ...
      <div className="md:col-span-2">
        ...
        {state && 'allergyRiskDetected' in state && (
          <Card>
            <CardHeader>
              <CardTitle>Allergy Check Results</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <Card className="bg-background/50">
                ...
                <CardContent>
                  <p className="text-muted-foreground">{state.alternativeMedicationOptions}</p>
                </CardContent>
              </Card>
              ...
              {mode === 'pharmacist' && (
                <Card className="bg-background/50">
                  <CardHeader>
                    <CardTitle className="text-lg">Pharmacist Guidance</CardTitle>
                  </CardHeader>
                  <CardContent>
