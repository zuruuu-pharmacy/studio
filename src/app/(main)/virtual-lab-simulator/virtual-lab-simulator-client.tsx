...
export function VirtualLabSimulatorClient() {
  ...
  const [state, formAction] = useActionState<SopGeneratorOutput | { error: string } | null, FormData>(
    async (previousState, formData) => {
      ...
      try {
        ...
        console.error(e);
        return { error: "Failed to generate simulation. Please try again." };
      }
    },
    null
  );
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <div className="relative">
                            <Beaker className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                            <Input placeholder="e.g., Gram Staining of Bacteria..." className="pl-10" {...field} />
                        </div>
                      </FormControl>
                      <FormMessage />
                    </FormItem>
        ...
        {isPending && (
            <div className="flex justify-center items-center h-full">
                <div className="text-center space-y-2">
                    ...
                    <p className="text-muted-foreground">Generating Simulation for "{form.getValues('experimentTitle')}"...</p>
        ...
        {state && 'title' in state ? (
          <Card>
            ...
            <CardContent className="space-y-6">
                ...
                <Accordion type="multiple" className="w-full space-y-4" defaultValue={['simulation', 'procedure']}>
          ...
          !isPending && (
             <Card className="flex flex-col items-center justify-center h-full min-h-[300px] text-center p-6 bg-muted/50">
                <Beaker className="h-16 w-16 text-muted-foreground/50 mb-4" />
                <h3 className="text-xl font-semibold text-muted-foreground">Your Simulation Will Appear Here</h3>
                <p className="text-muted-foreground/80 mt-2">Enter an experiment title to begin.</p>
            </Card>
          )
        )}
      </div>
